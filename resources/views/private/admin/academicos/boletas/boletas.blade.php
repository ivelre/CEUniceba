...
<script src="https://cdn.jsdelivr.net/npm/vue@2.5.16/dist/vue.js"></script>
    ...
    var app = new Vue({
      ...
      methods:{
        getKardex:function(){
          axios.get('/test-controlescolar/admin/academicos/kardex/calificaciones/' + app.estudiante.estudiante_id).then(function(response) {
        ...
        calPromedio:function(){
          ...
          for (var i = 0; i < app.materias.length; i++) {
            if(sumas[app.materias[i].semestre] === undefined){
              if(Number(app.materias[i].calificacion) >= 7)
                sumas[app.materias[i].semestre] = {suma:Number(app.materias[i].calificacion),materias:1,aprobadas:1,reprobadas:0}
              else
                sumas[app.materias[i].semestre] = {suma:Number(app.materias[i].calificacion),materias:1,aprobadas:0,reprobadas:1}
            }
            else{
        ...
        printCertificado:function(tipo_certificado){
          ...
          window.open('/test-controlescolar/admin/pdf/certificado/'+ tipo_certificado + '/'+ app.estudiante.estudiante_id + '/'+ app.certificado + '/'+ $('#fecha_certificado').val() ,'_blank')
        },
        getAsignaturas: function(){
          axios.get('/test-controlescolar/admin/academicos/kardex/reticula/' + this.estudiante.plan_especialidad_id).then(response => {
    ...
    function getData(value){
      axios.get('/test-controlescolar/admin/academicos/kardex/' + value).then(function(response) {
